...
import { conventionTermApi } from '../../api/conventionTerm';
import { llmApi } from '../../api/llm';
import type { ConventionTerm, ConventionTermCreate, ConventionTermUpdate } from '../../types/convention';

dayjs.locale('zh-cn');

type SlangTerm = ConventionTerm;

const categoryOptions = [
  { label: '全部分类', value: '' },
  { label: '展会类型', value: '展会类型' },
  { label: '票务术语', value: '票务术语' },
  { label: '活动术语', value: '活动术语' },
...
const ConventionTermList: React.FC = () => {
  ...
  const handleSaveContent = async (field: 'ai_copywriting' | 'ai_script', content: string) => {
    ...
    try {
      await conventionTermApi.updateAiContent(assistTerm.id, { [field]: content });
  ...
  const fetchTerms = async () => {
    ...
    setError(null);
    try {
      const data = await conventionTermApi.getTerms({
      ...
      // 获取统计数据
      const stats = await conventionTermApi.getTermStats(categoryFilter || undefined);
  ...
  const handleDelete = (id: string) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await conventionTermApi.deleteTerm(id);
  ...
  const handleStatusChange = async (id: string, checked: boolean) => {
    try {
      await conventionTermApi.updateTerm(id, { is_active: checked });
  ...
  const handleVideoGeneratedChange = async (id: string) => {
    try {
      const updated = await conventionTermApi.toggleVideoGenerated(id);
      setTerms(prev => prev.map(t =>
  ...
  const handleVideoPublishedChange = async (id: string) => {
    try {
      const updated = await conventionTermApi.toggleVideoPublished(id);
      setTerms(prev => prev.map(t =>
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editingTerm) {
        const updateData: ConventionTermUpdate = {
          term: values.term,
        ...
        const updated = await conventionTermApi.updateTerm(editingTerm.id, updateData);
        setTerms(prev => prev.map(t =>
        ...
        const createData: ConventionTermCreate = {
          term: values.term,
          meaning: values.meaning,
          usage_scenario: values.usage_scenario,
          category: values.category,
        ...
        const created = await conventionTermApi.createTerm(createData);
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `漫展圈黑话术语_${dayjs().format('YYYY-MM-DD')}.json`;
    link.click();
  ...
  return (
    <div className="fade-in">
      ...
      <div className="page-header" style={{ padding: '20px 24px 16px' }}>
        <h3 className="page-title">漫展圈黑话术语库</h3>
      ...
      >
        ...
        >
          ...
          >
            <Input placeholder="如：漫展、COS、自由行等" />
          </Form.Item>
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <Tag color="cyan" style={{ margin: 0 }}>{assistTerm?.term}</Tag>
...
export default ConventionTermList;
