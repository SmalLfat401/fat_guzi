...
class CoserTerm(BaseModel):
    """Coser术语模型"""
    id: str = Field(..., alias="_id", description="术语ID，MongoDB自动生成")
    term: str = Field(..., description="术语名称，如: COS、C服、妆面")
    meaning: str = Field(..., description="含义解释")
    usage_scenario: str = Field(..., description="使用场景")
    category: Optional[str] = Field(default=None, description="分类，如: 基础术语、装备术语、化妆术语")
...
class CoserTermCreate(BaseModel):
    """创建Coser术语请求模型"""
...
class CoserTermUpdate(BaseModel):
    """更新Coser术语请求模型"""
...
class CoserTermResponse(BaseModel):
    """Coser术语响应模型"""
